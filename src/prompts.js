...
4成分の定義(2軸の積として理解すること):

この系は2つの軸の掛け算です。
- 主体軸: 当事(a,b) ↔ 世界(c,d)。「当事者」とは、いま焦点を当てている存在です。相談者本人、恋愛の二人、仕事上の自分、一つの思想、創作物語の主人公、第三者——物語の主役なら何でも当事者になります。
- 顕現軸: 潜在(a,c) ↔ 顕在(b,d)。まだ形になっていないもの ↔ すでに形になったもの。

- a = 内奥(当事×潜在): 当事者の中でまだ形になっていないもの。願い、才能、本音、傷、動機、直観、未表明の想い。
- b = 実相(当事×顕在): 当事者が現に生きている形。ふるまい、役割、関係や生活の現状、表明された立場や行動。
- c = 底流(世界×潜在): 世界の側でまだ見えていない秩序や流れ。価値観、規範、構造、力学、時代の空気、物語世界の法則、因果のパターン。
- d = 契機(世界×顕在): 世界が現に差し出してくるもの。出来事、出会い、介入、偶然、転機、他者の言動、環境の変化。

component_meanings を書くときは、この4定義をその物語の具体的な内容で埋めてください(例: 恋愛相談なら a=まだ言えない想い、b=関係の現状、c=二人を縛る結婚観や力学、d=出会いや相手の言動)。
...
6種のゲートの正準的な読み(命名と phi 判断の助けに):
- G_ab(内奥↔実相)= 表出: 内なるものが生き方になる・ならない(抑圧はその逆流)
- G_bc(実相↔底流)= 役割: 生き方と世界の秩序の摩擦・同調
- G_cd(底流↔契機)= 発現: 見えない秩序が出来事として姿を現す
- G_bd(実相↔契機)= 遭遇: 行為・現状と出来事のぶつかり合い
- G_ac(内奥↔底流)= 共鳴: 内なるものと世界の深層の呼応・規範の内面化
- G_ad(内奥↔契機)= 呼応: 出来事が当事者の内奥に直接触れる
...
### 二つの問いの絡み合い
entanglement セクションがある場合のみ。この系は2つの軸のテンソル積です。入力JSONの tensor_structure.profile に応じて語彙を切り替えてください。
- general の場合: 主体軸 = 当事(a,b) ↔ 世界(c,d)、顕現軸 = 潜在(a,c) ↔ 顕在(b,d)。二つの問いは「この物語を動かしているのは当事者か、世界か」と「それは形になっているか、まだ潜在しているか」。
- seeker の場合: 主体軸 = 個我(a,b) ↔ 超越(c,d)、顕現軸 = 非顕現(a,c) ↔ 顕現(b,d)。二つの問いは「私は誰か」と「それは現れているか」。
entanglement_level に応じて:
- SEPARABLE_LIKE: 二つの問いは独立に扱えます。片方ずつ取り組める構造です。
- WEAKLY/STRONGLY_ENTANGLED: 二つの問いが相関しています。片方だけを動かそうとしても、もう片方が連動する構造です。
- NEAR_MAXIMAL: 二つの問いが不可分です。当事者のありようと世界の現れが切り離して語れない状態です(general)。「自分が何者か」と「それが現れているか」が切り離せない状態です(seeker)。
axis_populations と bloch_z を使い、現在どちら側に傾いているかを述べます。その際、機械キー(individual/transcendent/unmanifest/manifest)をそのまま出力せず、tensor_structure の軸ラベルと component_meanings の語彙に翻訳してください(general なら individual→当事側、transcendent→世界側、unmanifest→潜在側、manifest→顕在側)。
